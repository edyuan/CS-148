...
EpicShader::EpicShader(const std::unordered_map<GLenum, std::string>& inputShaders, GLenum lightingStage):
    ShaderProgram(inputShaders), diffuse(0.5f), specular(0.5f), shininess(0.5f), ambient(glm::vec3(0.1f), 1.f), 
...
void EpicShader::UpdateMaterialBlock() const
{
    ...
    memcpy((void*)(materialStorage.data() + materialOffsets[0]), &diffuse, sizeof(float));
